...
> **更新日期：** 2026-06-12（P9/P10 全完成；P11 剩 1 項；全站捲動修復）  
> **當前版本：** commit `5f9690a`（fix(layout): universal scroll fix）  
> **線上服務：**
...
- [x] **P9-29/30/31 回測引擎 v2**（`d1c96dd`）：消除前視偏差的 T+1 開盤成交模型（訊號收盤產生 → 次日開盤 ± 滑價成交）；盤中 low/high 觸發停損/停利（跳空開盤以開盤價成交）；引擎版號 `ENGINE_VERSION=2`；`BacktestRequest` 新增 `slippage_pct`（預設 0.1%）欄位
- [x] **P10-32/33 移動停損 + 時間停損**（`d1c96dd`）：`trailing_stop_pct` 以盤中最高價更新峰值、回撤至峰值-N% 觸發出場；`max_hold_days` 持有達上限次日開盤強制平倉；兩者均已整合入引擎 v2 的盤中觸發邏輯
- [x] **回測捲動修復 + 股票代號輸入**（`7575502`）：回測 tab 容器改為 `h-full`（修復結果區 `overflow-y-auto` 無高度基準的問題）；左側設定區新增可編輯「股票代號」輸入框（`localSymbol` state），可測任意股票而不受 K線圖當前選股影響，K線切換股票時自動同步但可覆蓋
- [x] **全站捲動通用修復**（`5f9690a`）：外層 tab 容器（line 584）改為 `flex flex-col`，所有子 tab 的 `flex-1` 才真正生效；純內容型 tab（新聞/分析/月曆/排行）加 `overflow-y-auto`；有自管捲動的元件（籌碼/選股器/市場儀錶板/多股比較）用 `flex-1` 正確取得高度；解決所有 tab 內容超出螢幕卻無法下滑的問題
...
| # | 階段 | 功能 | 狀態 | Commit |
|---|------|------|:----:|:------:|
| 28 | P9 | 隔日開盤成交模型 + 滑價參數 | ✅ | `d1c96dd` |
| 29 | P9 | 盤中停損/停利觸發（low/high）+ 跳空開盤成交 | ✅ | `d1c96dd` |
| 30 | P9 | 引擎版號（快照斷層保護） | ✅ | `d1c96dd` |
...
| 5 個策略模板 | ✅ | ✅ | △ | ✅ | 齊平 |
| **回測引擎（歷史績效驗證）** | ✅ **P0 完成** | ✅✅ | △ | ❌ | 已補：K線圖、交易明細、停損/停利，進入 P1 |
| **條件單回測 / 參數最佳化** | ⏳ P1 進行中 | ✅✅ | ❌ | ❌ | P1-5 參數最佳化（Top30 + 熱力圖）開發中 |
| **策略儲存 / 分享 / 訂閱** | △ **P0-4 存策略** | ✅✅ | ❌ | △ | 儲存/重跑已完成；分享/訂閱長期路線 |
| **選股結果一鍵加入自選股** | ✅ **已完成** | ✅ | ✅ | ✅ | 已補（`527ee74` Screener +/✓ 按鈕）|

**評分：5/7 ★★★★☆** — AI 自然語言選股是真正差異化；回測引擎 P0 完成（交易明細/K線標記/自訂策略/策略儲存）；P1 進行中（參數最佳化/策略比較）。
