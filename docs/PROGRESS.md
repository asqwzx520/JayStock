...
| **行動版 RWD（手機看盤）** | △ 很弱 | ✅✅ | App ✅ | △ | **台灣 60% 看盤在手機，這是留存率殺手** |
| **鍵盤快捷鍵** | ❌ | ✅✅ | △ | ❌ | 進階用戶需求 |
...
### 第 4 步：行動版 RWD
> 台灣 60%+ 投資人用手機看盤，這是留存率殺手

- 手機版（< 768px）：左側欄折疊 → 底部抽屜
- Header 在手機版折疊搜尋欄
- 主圖表佔滿寬度，指標選擇改為 dropdown
- 底部固定 Tab bar（走勢圖 / 籌碼 / 大盤 / 選股）

### 第 5 步：繪圖工具（技術分析必備）
- Lightweight Charts 支援 `createLineTool`
- 至少實作：水平線（阻力支撐）、趨勢線、矩形框選
- 可選：斐波納契回調線
